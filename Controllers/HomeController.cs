...
namespace MakeMyTripApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> UpdateReservation(int id)
        {
            ...
            {
                using (var response = await httpClient.GetAsync("http://localhost:5226/Api/Reservation/" + id))
                {
        ...
        public async Task<IActionResult> DeleteReservation(int ReservationId)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("http://localhost:5226/Api/Reservation/" + ReservationId))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
        ...
        public async Task<IActionResult> UpdateReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PutAsync("http://localhost:5226/Api/Reservation/", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> AddReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PostAsync("http://localhost:5226/Api/Reservation/", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    receivedReservation = JsonConvert.DeserializeObject<Reservation>(apiResponse);
                }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("http://localhost:5226/Api/Reservation/"))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
